    ...
    <div className="animate-fade w-full">
      ...
      <div className="mb-4 flex items-center gap-x-4">
        <input
          ...
          value={formData.selectedDate ? formData.selectedDate.split("-").reverse().join("-") : ""}
          onChange={(e) => {
            const [year, month, day] = e.target.value.split("-");
            const formattedDate = `${day}-${month}-${year}`;
            setFormData({ ...formData, selectedDate: formattedDate, selectedDoctor: null });
          }}
        />
      ...
      <div className="w-full flex justify-center my-4">
        <div className="w-full flex bg-gray-200 rounded-lg p-1">
          <button
            className={`flex-1 py-2 text-center rounded-lg text-sm font-medium transition-all ${
              mode === "online" ? "bg-blue-600 text-white shadow-md" : "text-gray-600"
            }`}
          ...
          <button
            className={`flex-1 py-2 text-center rounded-lg text-sm font-medium transition-all ${
              mode === "offline" ? "bg-blue-600 text-white shadow-md" : "text-gray-600"
            }`}
      ...
      <div className="flex flex-col gap-y-4">
        {doctors?.map((doctor, ix) => (
          <DoctorSlotCard key={`${doctor.id}-${mode}`} data={doctor} formData={formData} setFormData={setFormData} mode={mode} />
        ))}
        {doctors.length === 0 && <div className="my-12 text-center text-gray-500">No doctors available for this date!</div>}
      </div>
    </div>
  );
}
